...
IceCreamSundae::IceCreamSundae(Toppings::ToppingsList toppings) :
	...
	m_toppings(toppings),
	m_description() {
	wcout << L"Parameters Constructing IceCreamSundae(\Toppings::ToppingsList)." << endl;
}
...
const wchar_t* IceCreamSundae::GetSundaeDescription(void) {
	wstringstream str;
	str << L"A " << GetFlavorString(m_flavor) << L" sundae with the following toppings: " << m_toppings.GetString();
